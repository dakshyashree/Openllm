...
def login_page():
    ...
    with col2:
        st.markdown(
            "<h2 style='text-align: center;'>Secure Access</h2>", unsafe_allow_html=True
        )
        st.markdown(
            "<p style='text-align: center;'>Please enter your credentials to access the OpenLLM platform.</p>",
            ...
            if login_button:
                ...
                if success:
                    ...
                    st.session_state.page = (
                        "Main App"  # Redirect to main app after login
                    )
                    st.rerun()
                else:
                    st.error(message)
...
def register_page():
    ...
    with col2:
        st.markdown(
            "<h2 style='text-align: center;'>Create OpenLLM Account</h2>",
            unsafe_allow_html=True,
        )
        st.markdown(
        ...
        if admin_slots_available > 0:
            role_options_labels.insert(
                0, "Admin"
            )  # Add Admin as the first option if available
            st.info(
                f"Admin slots available: **{admin_slots_available}** out of **{MAX_ADMIN_COUNT}**."
            )
            default_role_index = (
                0  # Admin will be the default pre-selection if available
            )
        ...
        with st.form("register_form"):
            ...
            if register_button:
                ...
                success, message = register_user(
                    new_username, new_password, selected_role
                )
                if success:
                    st.success(message)
...
def admin_user_management_page():
    ...
    """
    st.markdown(
        f"<h1 style='text-align: center; color: var(--primary-color);'>Admin User Management</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
    ...
    if users_data:
        ...
        col_select_status, col_status_toggle = st.columns(
            [3, 2]
        ...
        with col_select_status:
            ...
            users_for_status_change = [
                u for u in users_data.keys()
            ]  # All users for selection, logic handles self-deactivation
            selected_user_status = st.selectbox(
                "Select User to Change Status",
                options=users_for_status_change,
                key="select_user_to_manage_status",
            )

        # Only show controls if a user is selected
        if selected_user_status:
            current_active_status = users_data.get(selected_user_status, {}).get(
                "active", True
            )
            ...
            with st.container(border=True):  # Use a container for the form-like input
                ...
                new_status = st.checkbox(
                    f"Set '{selected_user_status}' to Active",
                    value=current_active_status,
                    key=f"user_status_checkbox_{selected_user_status}",
                )

                if st.button(
                    f"Apply Status Change",
                    key=f"update_status_button_{selected_user_status}",
                    use_container_width=True,
                ):
                    # Prevent admin from deactivating themselves if they are the sole active admin
                    if (
                        selected_user_status == st.session_state.username
                        and not new_status
                    ):
                        active_admins_count = sum(
                            ...
                            and u != selected_user_status
                        )
                        if (
                            users_data[selected_user_status]["role"] == "admin"
                            and active_admins_count == 0
                        ):
                            st.error(
                            ...
                            success, msg = update_user_status(
                                selected_user_status, new_status
                            )
                            if success:
                                st.success(msg)
                                st.rerun()
        ...
        with col_select_delete:
            ...
            users_to_delete_options = [
                u for u in users_data.keys() if u != st.session_state.username
            ]
            if not users_to_delete_options:
                ...
                selected_user_delete = st.selectbox(
                    ...
                    options=users_to_delete_options,
        ...
        if selected_user_delete:
            with col_button_delete:
                ...
                st.write("")
                if st.button(
                    f"Delete '{selected_user_delete}'",
                    key=f"delete_user_button_{selected_user_delete}",
                    use_container_width=True,
                    type="secondary",
                ):
                    ...
                    success, msg = delete_user(
                        selected_user_delete, st.session_state.username
                    )
                    if success:
                        st.success(msg)
                        st.rerun()
...
def main_rag_app_page():
    ...
    """
    st.markdown(
        f"<h1 style='text-align: center; color: var(--primary-color);'>OpenLLM Insight Platform</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
    ...
    if st.session_state.role == "admin":
        ...
        with st.container(border=True):
            ...
            if zip_file:
                ...
                temp_extract_dir = (
                    Path("uploaded_files")
                    / f"temp_zip_{zip_name_stem}_{os.urandom(4).hex()}"
                )
                temp_extract_dir.mkdir(parents=True, exist_ok=True)

                st.info(
                    f"Extracting '{zip_file.name}' to a temporary location for processing..."
                )
                try:
                    with zipfile.ZipFile(zip_file, "r") as z:
                        z.extractall(
                            temp_extract_dir
                        )  # Extracts with internal structure into temp_extract_dir

                    # List all files extracted, including those in subdirectories
                    files_to_process = [
                        p for p in temp_extract_dir.rglob("*") if p.is_file()
                    ]
                    st.success(
                        f"Successfully extracted {len(files_to_process)} items from '{zip_file.name}'."
                    )
                    ...
                    for doc_path_in_temp_dir in files_to_process:
                        ...
                        while final_doc_path.exists():
                            final_doc_path = (
                                original_final_doc_path.parent
                                / f"{original_final_doc_path.stem}_{counter}{original_final_doc_path.suffix}"
                            )
                            counter += 1
                        ...
                        try:
                            ...
                            # Ingestion
                            ingest_to_faiss_per_file(
                                final_doc_path, base_dir="document_index"
                            )
                            st.success(f"Ingested `{final_doc_path.name}` into FAISS.")

                            # Always attempt summarization
                            with st.spinner(
                                f"Generating summary for `{final_doc_path.name}`..."
                            ):
                                try:
                                    summary = summarize_file(final_doc_path)
                                    st.markdown(
                                        f"**Summary for `{final_doc_path.name}`:**\n{summary}"
                                    )
                                    st.success(
                                        f"Summary generated for `{final_doc_path.name}`."
                                    )
                            ...
                            st.error(
                                f"Failed to process `{doc_path_in_temp_dir.name}`: {e}"
                            )
                    ...
                    if failed_files:
                        ...
                        st.success(
                            f"All {processed_count} files from '{zip_file.name}' processed successfully!"
                        )
                    ...
                    if temp_extract_dir.exists():
                        st.info(
                            f"Cleaning up temporary extracted directory: `{temp_extract_dir}`..."
                        )
                        try:
                            ...
                            st.warning(
                                f"⚠ Could not remove temporary directory `{temp_extract_dir}`: {e}"
                            )

            st.markdown("---")
            st.markdown("### Upload Individual Files")
            st.info(
                "Alternatively, you can select one or more individual files from different locations."
            )
            ...
            if uploaded_file_paths:
                st.write(
                    f"Initiating processing for {len(uploaded_file_paths)} individual file(s)..."
                )
                for (
                    file_path
                ) in uploaded_file_paths:  # Iterate through each uploaded file
                    stem = file_path.stem

                    st.info(
                        f"▶ Ingesting `{file_path.name}` into FAISS… This may take a moment."
                    )
                    try:
    ...
    st.markdown(
        "<p>Ask a question across all available document summaries to gain comprehensive insights.</p>",
        unsafe_allow_html=True,
    )

    with st.container(border=True):
        general_q = st.text_input(
            "Enter your global question:",
            key="qa_global_question",
            placeholder="e.g., What are the common themes across all reports?",
        )

        if st.button(
            "Run Global QA", key="run_global_qa_button", use_container_width=True
        ):
            summary_dir = Path("uploaded_files")
            summary_paths = list(summary_dir.glob("*.summary.txt"))
            if not summary_paths:
                st.error(
                    "No summary files found. Ensure documents have been uploaded and summarized."
                )
            else:
                docs = []
                ...
                try:
                    with st.spinner(
                        "Building global knowledge base and finding relevant information..."
                    ):
                        ...
                        if not top:
                            st.warning(
                                "No relevant document found for your global question. Consider rephrasing."
                            )
                        else:
                            ...
                            if not matches:
                                ...
                                st.session_state.global_qa_history.append(
                                    (general_q, chosen_stem, answer)
                                )
                                st.markdown(
                                    f"**Best Match Document:** \n`{chosen_stem}` \n**Answer:** {answer}"
                                )
                                st.success("Global query complete.")
                except Exception as e:
                    st.error(
                        f"Global QA Error: {e}. Ensure your OpenAI API key is valid and agents are working."
                    )
...
st.markdown(
    "<h1 style='text-align: center; color: var(--primary-color);'>Welcome to OpenLLM</h1>",
    unsafe_allow_html=True,
)
st.markdown(
...
if st.session_state.logged_in:
    ...
    selected_page = st.sidebar.radio(
        ...
        index=(
            pages.index(st.session_state.page) if st.session_state.page in pages else 0
        ),
    ...
    with st.sidebar.expander("Change Password"):
        ...
        current_password = st.text_input(
            "Current Password", type="password", key="sidebar_current_password_input"
        )
        new_password = st.text_input(
            "New Password", type="password", key="sidebar_new_password_input"
        )
        ...
        if st.button(
            "Update Password",
            use_container_width=True,
            key="sidebar_update_password_button",
        ):
